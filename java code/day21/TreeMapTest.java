...
public class TreeMapTest{
    public static void main(String[] args){
        /*
        练习二：学生对象（姓名，年龄）都有自己的归属地，既然有对应的关系。将学生对象和归属地存储到map集合中。
        注意：同姓名同年龄视为重复的键。按照学生的年龄进行从小到大的排序。TreeMap
...
/*
在Map.Entry<Student,String> 中，getKey()和getValue() 的区别很简单：

getKey():获取当前键值对中的键（这里是Student 对象）
getValue():获取当前键值对中的值（这里是String对象）

两者的类型不同，含义不同。一个代表映射的“索引”，一个代表“数据”。在遍历Map时，需要分别取出键和值进行后续操作。
*/