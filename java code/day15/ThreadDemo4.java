...
同步函数使用的锁是哪个？
经过分析：大概是this，因为函数必须被对象调用。

验证：
写一个同步代码块，写一个同步函数，如果同步代码块中的锁对象和同步函数中的锁对象是同一个，就同步了，就没有错误的数据了，
如果不是同一个锁对象，就不同步，出现错误数据。
...
同步代码块用的锁是this。当只需要一个同步时，用同步函数。当有多个同步时，要用同步代码块！同步代码块的锁
可以是任意对象，
*/
...
//3月8日  复习
/*
同步函数的锁是this，当只需要一个同步时，用同步函数。当需要多个同步函数时，用同步函数代码块！同步代码块的
锁可以是任意对象！
*/