<template>
  <div class="detail-page">
    ...
    <div class="detail-card" v-if="bookInfo">
       <!-- v-if="bookInfo"：判断是否成功匹配到图书
匹配到图书（bookInfo 有值）：展示封面、书名、作者、分类、库存、简介、返回按钮
v-else：匹配失败 / 加载中，显示「加载图书信息中...」-->
      <div class="img-box">
      ...
      <div class="info-box">
        ...
        <button class="back-btn" @click="$router.back()">返回上一页</button><!--点击按钮执行路由返回方法，回到之前的图书列表页面。-->
      </div>
    </div>
...
// 1. 获取当前页面路由信息（包含网址所有内容）
const bookInfo = ref(null)
// 2. 提取网址路径上 :id 对应的数值

// 根据路由id匹配对应图书
const bookId = Number(route.params.id)
//此时 rawId 是字符串 "2"，不是数字，所以转一下：
bookInfo.value = allBooks.find(item => item.id === bookId)
//allBooks 是所有图书的总数组，循环对比每本书的 id，找到和网址传过来的 bookId 一模一样的那本书，存入 bookInfo。
//问：find 和 filter 有什么区别？
//答：find找到第一条符合数据就停止，返回单个对象，适合根据唯一 id 找单本图书；filter遍历全部数据，返回所有符合条件的数组，适合分类筛选多本书。
</script>
